...
# ── 2c. Serving the node's name ───────────────────────────────────────────────
# The node is reached at the name the cluster's DNS anchor gives it, served from the site this API
# generates. Root-owned like the rest of /etc/nginx, which is why it lives here and not in deploy.sh;
# skipped cleanly when nginx is not installed.
setup_serving
