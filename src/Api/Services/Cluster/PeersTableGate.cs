...
/// service token is enabled unless its row's <see cref="Data.PeerEntity.Enabled"/> flag is explicitly
/// <see langword="false"/>. Absence from the roster is not rejection — only an explicit disable is, because
...
/// </remarks>
public sealed class PeersTableGate(PeersStore peers, ILogger<PeersTableGate> logger) : IClusterMemberGate
{
    /// <inheritdoc/>
    public async Task<bool> IsEnabledAsync(string memberId)
    {
        Data.PeerEntity? row = await peers.GetByNodeIdAsync(memberId, CancellationToken.None).ConfigureAwait(false);
