...
public sealed class LeafDescriptorStore(ApiOptions options, ILogger<LeafDescriptorStore> logger)
{
    ...
    /// <summary>
    /// Every leaf descriptor installed on this host — the components this NODE runs. An anchor is not
    /// among them and needs no excluding: it serves one capability to the whole cluster and is a peer of
    /// this node rather than something it hosts, so it is described in its own directory and this scan
    /// never sees it.
    /// </summary>
