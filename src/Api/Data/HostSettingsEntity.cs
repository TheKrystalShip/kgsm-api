...
/// back to config" (the <c>Api__HostLabel</c>/<c>Api__Region</c> deploy-time default), never a
...
public sealed class HostSettingsEntity
{
    ...
    /// <summary>Display label override. Null ⇒ use the configured <c>Api__HostLabel</c>.</summary>
    public string? Label { get; set; }

    /// <summary>Deployment region override — an arbitrary free string (e.g. <c>eu-west</c>). Null ⇒ use the
    /// configured <c>Api__Region</c> (which is itself null when unset — honest unknown, never guessed).</summary>
