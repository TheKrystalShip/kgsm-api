    ...
    ClusterPeerRelay relay) : ControllerBase
{
    ...
    /// (PLAN-peers.md §2·b, G4). Cluster-token authed + disable-list gated, same fail-closed posture as the
    /// cluster package's inbox; deliberately separate from the durable message bus — a best-effort push-pull, no
