...
/// <summary>
/// WS delivery for the live roster (player-presence-contract.md §5), in-process against the real
/// pipeline — the same pattern as <c>AuditTests.AuditTopic_DeliversAppend</c>: seed through
/// <see cref="PlayerRosterService"/> directly (resolved off the running app's DI, same singleton
/// <c>KgsmAuditConsumer</c> would drive) and assert what the real <c>StreamHub</c> pushes over a real
/// WebSocket. Covers the <c>players.reset</c> frame's gate — only published when the reset actually
/// removed a session, never for an already-empty roster.
/// </summary>
public sealed class PlayerRosterWsTests(AuthTestFactory factory) : IClassFixture<AuthTestFactory>
{
    private PlayerRosterService Roster => factory.Services.GetRequiredService<PlayerRosterService>();
    ...
    public async Task Reset_NonEmptyRoster_PublishesPlayersReset_WithServerId()
    {
        ...
        Roster.Join(serverId, "sess-1", "id-1", "Heisen", null, DateTimeOffset.UtcNow);
        ...
        {
            Roster.Join(serverId, "sess-1", "id-1", "Heisen", null, DateTimeOffset.UtcNow); // re-arm if consumed
            Roster.Reset(serverId);
