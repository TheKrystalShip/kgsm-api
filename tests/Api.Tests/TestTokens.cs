...
internal static class TestTokens
{
    ...
    /// <see cref="AuthClaims.SessionId"/> (<c>sid</c>) claim — a faithful pre-M4·c token. JwtBearer
    ...
    /// </summary>
    public static string MintAccessWithoutSidClaim(string signingKey, string hostId, AuthTier tier)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(AuthClaims.Host, hostId),
            new(AuthClaims.TokenKind, TokenKind.Access),
            // NO sid claim — the whole point (pre-M4·c token simulation).
            new(AuthClaims.Username, FakeDiscordResolver.Identity.Username),
            new(AuthClaims.Display, FakeDiscordResolver.Identity.Display),
            new("scope", string.Join(' ', FakeDiscordResolver.Identity.Scopes)),
        };
        if (FakeDiscordResolver.Identity.AvatarUrl is not null)
            claims.Add(new Claim(AuthClaims.Avatar, FakeDiscordResolver.Identity.AvatarUrl));
