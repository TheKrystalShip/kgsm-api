...
- **Upstream piece (kgsm-lib 1.15.0 → 1.16.0):** the blueprint surface emits `Ports` only as the legacy
  UFW string (`"26900:26903/tcp|26900:26903/udp"`) — unlike `instances info --json`, which is already
  structured. Rather than re-parse an opaque port string in the api (which `PortMapping`'s own contract
  forbids — "no consumer re-parses a port string"), kgsm-lib gained **`PortMappingExtensions.FromUfwSpec`**
  (the inverse of `ToUfwSpec`): single port / inclusive range / proto-less → tcp+udp, malformed entries
  skipped defensively. The catalog gets a structured value at the chokepoint; the SPA never splits a string.
- **Built (2026-06-19):** `Contracts/LibraryDto.cs` (the frozen shape above), `Services/Library/LibraryAggregator.cs`
  ...
  `Controllers/LibraryController.cs` (`GET /library`, viewer). The api's kgsm-lib ref bumped 1.15.0 → 1.16.0.
- **Self-validated:** `scripts/smoke.sh` → **44/44** (+2 M8·a: a **live read of the real 29-blueprint
  dev catalog** asserting the frozen key set, structured ports from `FromUfwSpec`, steam null-honesty
  both ways, and reserved cover/rawgSlug; the `q` filter — `factorio`→matches, a no-match→`[]`; the
  no-token sweep now covers `/library` → 401). **`tests/Api.Tests` → 120** (+11 `LibraryTests`: the
  Blueprint→`LibraryEntry` mapping honesty — null-not-zero steam, id-fallback name, reserved cover/rawgSlug,
  structured + proto-less ports, curated-metadata override, the q filter, id ordering, and the
  engine-unconfigured / read-failure degrade-to-empty; + the viewer/none/no-token gate through the real
  pipeline). kgsm-lib → **512** (+6 `FromUfwSpec`). Release **0-warning**.
...
**The one genuine upstream piece (kgsm-lib 1.15.0 → 1.16.0).** The blueprint surface emits `Ports` only
as the legacy UFW string (`"26900:26903/tcp|26900:26903/udp"`) — unlike `instances info --json`, which is
already structured. `PortMapping`'s own contract forbids a consumer re-parsing a port string, so the parse
belongs at the chokepoint: kgsm-lib gained `PortMappingExtensions.FromUfwSpec` (the inverse of `ToUfwSpec`
— single port / inclusive range / proto-less → tcp+udp; malformed entries skipped defensively; null/blank →
empty). +6 tests (`PortMappingTests`), 512 green, packed to `/home/heisen/local-nuget`.
...
pattern; map + sort by id + `q` filter), `Controllers/LibraryController.cs` (viewer). kgsm-lib ref
1.15.0 → 1.16.0.

**Self-validated:** `scripts/smoke.sh` → **44/44** (+2 M8·a): a **live read of the real 29-blueprint dev
catalog** asserting the frozen key set, structured ports from `FromUfwSpec`, steam null-honesty both ways
(≥1 non-null + ≥1 null `steamAppId`), a real multi-port range parsed (`saw_range`), and reserved
cover/rawgSlug; plus the `q` filter (`factorio`→matches, a no-match→`[]`) and `/library` in the no-token
401 sweep. **`tests/Api.Tests` → 120** (+11 `LibraryTests`): the Blueprint→`LibraryEntry` mapping honesty
(null-not-zero steam, id-fallback name, curated-metadata override, structured + proto-less ports, reserved
cover/rawgSlug, the q filter, id ordering, engine-unconfigured + read-failure degrade-to-empty) + the
viewer/none/no-token gate through the real pipeline. Release **0-warning**.
