...
realtime `GET /api/v1/stream` fetch-based SSE stream** (per-host topics pushed by gated pumps + an always-on
...
`[Authorize]` on every prior endpoint, auth **on by default** with an explicit `Api__AuthDisabled=true`
...
independent v0.3 signals: per-host SSE stream (§3·b), per-host auth/bot (§3·f),
...
- **Time-series metrics *history*** (`GET /servers/{id}/metrics?range=1h`) needs a
  **metrics store** the monitor doesn't provide (it serves *latest* only). Treated
  as its own decision (§5; kgsm-monitor owns metrics history — consumer contract
  `kgsm-monitor/docs/integration.md`), not bundled into
...
### M2 — Realtime: SSE per host  ·  `partial` (backend built & self-validated 2026-06-15; transport
migrated WebSocket→SSE 2026-07-02, protocol in `src/Api/Realtime/CLAUDE.md`; frontend gate pending)
- **Goal:** push the M1 data instead of polling (resolves keystone O2 → SSE).
...
- **Scope:** `/api/v1/stream` (fetch-based `text/event-stream`, **unauthenticated until M4** — a
  pre-auth read surface); `{ topic, type, data }` envelope; topics chosen at connect via
  `?topics=a,b,c` (**immutable per connection** — no client→server channel, no
  subscribe/unsubscribe; changing topics means opening another stream); topics `servers`,
...
- **Built:** the full M2 contract is frozen in §6. Hand-rolled SSE (not SignalR — the
  hand-rolled `{topic,type,data}` envelope is the contract; fetch-based, not native `EventSource`,
  so the bearer can ride a header); central `Realtime/StreamProtocol.cs`
  ...
  no snapshot-on-subscribe (the client hydrates via REST). One shared `MetricsMapping` makes an SSE
...
- **Self-validated:** `scripts/smoke.sh` → **25/25** (8 M0 + 4 M1·a + 6 M1·b + **7 M2**), incl. an
  SSE reader that subscribes, reads honest ticks, proves the `servers` topic stays quiet
...
- **Risk:** SSE lifecycle/backpressure and the message-envelope contract (the response-body write
  loop is bog-standard under JIT).
...
- **Coverage honesty (the live half, like M3's mutation happy path):** the **event-sourced append from a
  ...
  provenance) is a **trusted-host live-validate**. It needs only a readable `Api__KgsmJournalDir`; the api
...
**M6·a — Alerts (condition-mirror).** · `partial` (crash source built & self-validated + **LIVE-VALIDATED** 2026-06-16; contract PROPOSED — frontend sign-off pending. **Metrics-threshold source (increment 1) BUILT + unit-tested + LIVE-VALIDATED 2026-07-01** (474/474; dev instance vs real monitor: host-mem danger + srv-pids warn raise, vanished-row resolve — UNCOMMITTED, not yet deployed to :8097). See §6 + §8.)
...
- **Metric thresholds — increment 1, being wired (no longer deferred).** The `host-monitor`/`metrics`
  sources are being populated: a sustained host/per-server
  `Snapshot` breach folds into the same `AlertEngine` (the unchanged crash `Tick` plus a new
  `TickMetrics(snap, now)` seam), policy from appsettings `MetricsThresholds` + env (a baked default —
  DB-backed/panel-editable policy is increment 2, out of scope), host rules ON by default / per-server
  rules OFF (operator opt-in), id namespace `metric:<ruleKey>[:<ref-or-serverId>]`, new `AlertSurface.Host`.
  **Implementation in progress — not yet built, tested, or live-validated; see §8 for the increment log.**
  Still deferred (no honest source): leaf-down (already on the `capabilities.patch` axis — infrastructure,
  not a §3·c game-server condition), port-unreachable (no prober).
- **Depends:** M5 (the audit bridge), M6·0 (the watchdog crash events + `IWatchdogClient`); the metrics-
  threshold increment additionally depends on M1·a's `MonitorClient` (already DI-registered).
- **Done:** the live watchdog-crash → alert validate (LIVE-VALIDATED 2026-06-16 — real crash→raise→resolve
  →`actionId:null` auto-heal; §8). **Owed:** the frontend sign-off on the §6 contract divergences; the
  metrics-threshold increment's build/test/validate pass (§8).
...
- **Strategy — RESOLVED (O1, 2026-06-19): near-verbatim relay; §5·a shaping pushed
  ...
  not just the SPA. Spec: **`kgsm-llm/docs/wire-contract.md`** (the upstream work + staged
...
- **Auth — RESOLVED (trusted co-located relay, 2026-06-19):** the assistant's `/turn` requires its
  ...
  **relay-auth path** (kgsm-llm `64abac8`): a shared `Assistant:Relay:Secret` + forwarded Discord
  ...
  verified caller's Discord id from its JWT + the shared secret (`Api__AssistantRelaySecret`).
...
- **Depends:** M4 (auth); the assistant service's **upstream §5·a pass** (`kgsm-llm/docs/wire-contract.md`,
...
- **`/integrations/discord` — a provider-agnostic notification subsystem (§3·e). DECIDED + Increment A built.**
  ...
  - **Increment B — the delivery worker — BUILT & self-validated 2026-06-19** (live-validate with a real webhook
    ...
    update / install / backup) with an optional ops-role ping (`Settings["pingRoleId"]` + `allowed_mentions`
...
- ~~**Metrics-history store** (for `GET /servers/{id}/metrics?range=…`)~~ — **RESOLVED
  2026-06-24, decisions LOCKED → milestone M9.**
...
- ~~**Proxy-verbatim vs re-wrap** for the assistant SSE (O1)~~ — **RESOLVED at M7 (2026-06-19):
  near-verbatim relay; §5·a shaping pushed UPSTREAM into the assistant** (`kgsm-llm/docs/wire-contract.md`),
...
| `Server` DTO (honest realization) | **M1·b** | `architecture.html §3` — **frozen 2026-06-14.** `{ id, name, blueprint, status, version?, runtime, hostId, metrics? }`; `metrics:{ cpuPctCore, memBytes, ioReadBps?, ioWriteBps?, pids }` or **`null`**. Stable keys, explicit nulls. **Divergences from the §3 example (the negotiated honest-vs-aspirational contract):** `status` is `running\|stopped\|unknown` (from `Reading<InstanceRuntimeStatus>`), NOT `online\|offline\|updating\|crashed\|installing` (transitional states need the M3 job tracker + crash detection); `metrics.cpuPctCore` is **% of one core (can exceed 100)**, not `cpu` 0–100; `memBytes` replaces `ram{used,max}` (no honest memory limit); **omitted as unsourceable**: `players`, `ip`, `ram.max`, `updatedAt` (no state-change tracking until M2), and the curated `game` display name (we emit the real `blueprint` id — metadata curation deferred, never guessed). Join key: instance id (`monitor ServerMetrics.Id` == kgsm instance name == lib dict key). `/servers/{id}` == the list element shape (full detail later). **ADDITIVE (Monitor.Contracts 1.2.0):** `metrics.diskBytes?` — per-instance on-disk footprint (bytes), nullable, passed through 1:1 (slow-cadence working-dir walk). **ADDITIVE 2026-06-29 (Monitor.Contracts 1.3.0 — per-server NETWORK):** `metrics.rxBps?` / `metrics.txBps?` — per-instance network receive/transmit rate in **bytes/sec**, **nullable**. Now honestly sourced: the monitor's passive eBPF `cgroup/skb` byte meter on the instance cgroup — this **reverses** the prior "per-server network has no honest source / deliberately not emitted" note. `null` (never `0`) when not measurable — meter not set up, `cap_bpf` absent, or a container instance not under `kgsm.slice`; same nullable-passthrough contract as `ioReadBps`. Purely additive (within `/api/v1`), no break; rides both `GET /servers/{id}.metrics` and the `servers/{id}/metrics` WS tick via the single `MetricsMapping.ToServerMetrics` (byte-identical). | **ADDITIVE 2026-08-02 (server note):** `note?` — the operator-authored note, `{ body, updatedBy?, updatedAt? }` or **`null`** when the instance has no note. Carried on the list, the detail AND the `servers` stream (unlike `cover`/`hero`/`network`, which are detail-only): the dashboard tile renders a note, so a list row must carry it without a detail fetch. Engine-owned — it lives in the kgsm instance's `.config.ini` under `note` (base64) + `note_updated_by` + `note_updated_at`, kgsm-lib 1.48.0 owns the codec — so it travels with the instance and dies with it on uninstall. Attribution is honest-null for a note hand-written into the config rather than through a surface; the body renders regardless. **ADDITIVE 2026-08-05 (active job):** `activeJob?` — the long-running operation that owns the instance right now (`update`, `backup_create`, `backup_restore`), as the same `Job` object the `jobs` topic carries, or **`null`** when nothing is in flight. Rides the list, the detail AND the `servers` stream, and `DomainPump` diffs it, so a surface reads "this instance is busy" from a plain GET instead of only from the transition frame — a panel opened or reloaded mid-update shows the state, and so does a second operator's. Sourced from `JobRegistry.InFlightFor`, whose one-per-server slot is claimed by **either** a job this api issued **or** an update kgsm is running for another entrypoint (CLI/assistant/bot), observed via `instance_update_started`/`instance_update_finished` (kgsm 3.7.2-rc1) — so it is at most one honest record whoever started it, never a fabricated one. **It is NOT a status:** `status` stays the run-state vocabulary above and a surface joins the two itself (kgsm-web renders `updating` from this field, the same way it renders an in-flight install as a phantom).
| `Server.id` / `Server.name` — the identity split | **2026-08-23** | `id` is the kgsm instance id: auto-generated at install, path-safe, **immutable**, and the key every route, keyed store, audit row and metric series uses. `name` is the instance's `display_name` — free text somebody chose to read it by, **not unique**, changed at any time through the rename surface below, and never part of a path. An instance with no label of its own reads as its id, so `name` is never blank and never a prettier invented label. Sourced from kgsm-lib's `Instance.DisplayName` (kgsm-lib **6.1.0**), which makes the id-fallback itself, including for an instance whose library is offline and whose config cannot be read. A surface that must tell two identically-labelled servers apart shows the id beside the label. |
| `InstallRequest` — create takes a label, and optionally an id | **2026-08-23** | `POST /servers` body `name` is the **label** (free text → `display_name`), not an identifier. New optional `id` names the instance id explicitly, for a caller that must know it before the install finishes; it is validated by the engine (`^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$` + the live roster) and a bad or taken one is a **400** with kgsm's own detail, never an adjusted id. With no `id`, the backend assigns one: a path-safe slug derived from the label when the engine accepts it, otherwise the engine's own `blueprint`/`blueprint-NN`. The derived slug never fails a create — a label that collides or does not survive the charset simply falls through. The `202` job's `serverId` is the assigned id, as before. |
| `ServerDisplayName` surface (`PUT/DELETE /servers/{id}/display-name`) | **2026-08-23** | **Operator** both verbs; there is no `GET` — the current label is `name` on the `Server` DTO, on the list, the detail and the `servers` stream alike. `PUT {displayName, origin?}` → `200 { serverId, displayName }` **re-read from the engine**; **400** on an empty label (clearing is `DELETE`) or over 200 chars measured after sanitizing (**rejected, never truncated**); **404** for an unknown id — the route means the id and never resolves a label, since labels are not unique; **500** `engine_refused` carrying the engine's own detail; **503** when kgsm is not provisioned. `DELETE ?origin=` clears the label, after which the instance reads as its id and the response reports that id. Nothing on disk is renamed and the id is untouched, so every downstream store keyed on it keeps its history. No audit row written here: kgsm's `instance_display_name_changed` echo carries actor+origin and becomes the `server.rename` row, so a rename typed at the CLI is audited identically. The companion `instance_config_changed` naming `display_name` is dropped by both audit paths so one rename is one row. |
| `ServerNote` surface (`GET/PUT/DELETE /servers/{id}/note`) | **2026-08-02** | Viewer read / Operator write. `GET` → `{ serverId, note }` (`note: null` when unset). `PUT {body, origin?}` → 200 with the fresh `{serverId, note}`; **400** on an empty body (clearing is `DELETE`, so an emptied editor can't silently wipe a note) or over 600 chars measured after sanitizing (**rejected, never truncated**); **500** `engine_refused` with `details:{applied[], failedKey}` when a key write fails part-way — kgsm sets one key per call, so a partial state is real and is reported rather than hidden. `DELETE ?origin=` blanks the body but records who cleared it. No audit row written here: the engine's `instance_config_changed` echo carries actor+origin, and both audit paths drop the two attribution keys so one edit = one row. `PATCH /servers/{id}/config` refuses the note's keys and points here. |
...
| SSE stream envelope + topic/type vocabulary | **M2** | `architecture.html §3·b/§3·j` — **frozen 2026-06-15; transport migrated WebSocket→SSE 2026-07-02 (protocol: `src/Api/Realtime/CLAUDE.md`).** Endpoint `GET /api/v1/stream` (fetch-based SSE, `text/event-stream`; **unauthenticated until M4** — a pre-auth *read* surface, less severe than M3's mutation but flagged). **Topics chosen at connect** via `?topics=a,b,c` (comma-separated; **immutable per connection** — no client→server channel, changing topics means opening another stream); unknown/future topics accepted silently (forward-compat for `jobs` M3 / `audit` M5 / `alerts` / `console`); **no ack or error-frame protocol.** **Outbound:** `data: <json>\n\n` where `<json>` is `{ topic, type, data }`, patch-only (the client `hydrate(REST) + applyPatch(stream)`; **no snapshot on subscribe** — §3·j re-hydrates via REST on (re)connect). **Topics (M1-backable subset):** `servers` · `servers/{id}/metrics` · `hosts/{id}/metrics` · `hosts/{id}/capabilities`. **Message types** (only `server.patch` is doc-given; the rest are ours, negotiated like the M1·b DTO — all centralized in `Realtime/StreamProtocol.cs`, never inline strings): `server.patch` (data = the **frozen M1·b `Server`**, NOT the §3·b example's `{status:"online", players}`; carries the full element incl. a **point-in-time `metrics` block that may lag** the dedicated `servers/{id}/metrics` tick — merge by id), `server.removed` (`{ id }` tombstone), `metrics.tick` (`ServerMetricsDto`), `host.metrics` (`HostMetricsDto` = the capacity portion of the `Host` view; `net`/`temp` omitted, never fabricated), `capabilities.patch` (`HostCapabilities`). **`servers` carries status/roster only — NOT the 1s metric firehose** (resource ticks live on `servers/{id}/metrics`; a deliberate divergence from §3·b's "resource deltas" wording, smoke-proven the `servers` topic stays quiet under ticking metrics). **Honesty:** monitor-down → metric topics go **silent** (never a replayed stale frame) and `hosts/{id}/capabilities` flips metrics `down` — that flip is what explains the silence. **Capability availability is driven by the always-on `LeafHealthMonitor`** (frequent `/health` polls, the single source feeding both this stream and the REST `GET /hosts`), which stamps `since` on each flip — see the Capability-record row. The `capabilities.patch` keeps `provisioned:true` through a down→up cycle: degrade **and** recover gracefully, capability never "lost". |
| `me` stream topic + `me.patch` | **2026-08-25** | The live half of `GET /api/v1/me`. Topic **`me`**, one message type **`me.patch`**, `data = { tier, status }` — the mutable half of the `/me` response in the same wire vocabulary (`tier` a `KgsmTiers` string incl. `none`; `status` a `UserStatuses` one or `unknown`). **Per-user delivery, never broadcast:** the frame reaches only the connections authenticated as the account it is about, matched on the **account id** so a session established through a linked provider identity is reached like any other; a connection proving no account here receives nothing. **Subscribe by adding `me` to `?topics=`**, and it is the one topic that needs no tier — `/stream` gates per topic (`StreamProtocol.MinimumTier`: operator for the host logs/services topics, none for `me`, viewer for everything else), so a pending account connects and holds `me` alone. **Pushed by every write that changes what an account may do** (`PATCH`/`DELETE /auth/users/{id}`, the notification approve action) via `StreamHub.AuthorityChanged`, which **also re-gates the live connection** — the tier moves and every subscription above it is dropped, so a demoted operator loses the operator topics and gets the redacted audit variant without reconnecting. A promotion adds no subscription back (the client reconnects asking for more). The connection re-reads its own standing every 20s as the backstop for writers this process never sees (the account store is a shared host file); an unreadable store **ends the stream** rather than resolving to `none`. |
...
| Leaf commands (`GET /hosts/{id}/services/{leaf}/commands`) | **leaf-config** | `../leaf-command-manifest.md` — **frozen 2026-08-05.** The catalog of commands a leaf answers to, read from the manifest that leaf's own `deploy.sh` installs into `/var/lib/kgsm/leaves/commands/<leaf>.json` (one directory below the config descriptors, because the descriptor scan globs `*.json` at the level above and would read it as a malformed one). `LeafCommandStore` **scans** that subdirectory on the same 30s TTL, so a leaf that grows a command surface is documented by landing one file — no rebuild here and no list of command-taking leaves anywhere. → `{ schemaVersion, leaf, surface, gate, commands:[{ name, description, mutates, options:[{ name, description?, type, required, autocomplete }] }] }`, served **verbatim**: this API has no idea what any command does, and passes the leaf's `gate` through without restating it because it cannot verify a check it does not implement. A leaf that ships no manifest is a **404**, not an empty list — most leaves take no commands, which is a different statement. Gated at **operator** with the rest of the read-only Services surface (`ServicesController`), not at the admin config gate: it is reference material, and nothing here mutates. A malformed or unknown-version file is skipped with its reason logged once per revision, never partially applied. Producer today is **kgsm-bot**, whose build generates the manifest from its own assembly. Built + self-validated 2026-08-05 (`LeafCommandsApiTests`, 10 tests: passthrough, empty-options normalization, the no-manifest/foreign-host 404s, four untrustworthy-file cases, isolation between leaves, and the 401/403/operator/admin gate matrix; live-read on this host → 11 bot commands, monitor 404). |
| Per-account preferences (`/me/preferences`) | **preferences** | negotiated with kgsm-web — **frozen 2026-08-21.** The general store the panel's editable surface reads: `GET /me/preferences` → `{ deviceId, sync:{ enabled, sourceDevice?, updated? }, preferences:{ <key>: { key, value, version, originDevice, updated } } }`, `PUT /me/preferences/{key}` `{ value }` → the stored record, `GET|PUT /me/preferences/sync` `{ enabled }` → the switch. **Keys are opaque** — `value` is the client's own JSON handed back verbatim, so a new preference (`dashboard.layout`, `ui.theme`, `ui.density`) is zero backend work; the API holds no idea what a widget is. **Device identity is client-minted and carried in `X-Krystal-Device`** (a session is per-host and expires, so a session id names a sign-in rather than a machine); a device-scoped call without it is `400 device_required`, never a default — the empty device is the synced record's own slot. `version` is monotonic per (account, key) and `originDevice` breaks a tie at equal versions, lexically: the merge key ships from the start although nothing propagates between nodes yet, because retrofitting one onto existing rows means inventing a version for each. `updated` is display only. Gated `[Authorize]`, not a tier — a person's own settings are self-service, so a `none`-tier caller awaiting approval still arranges their own panel. Caps: `413 value_too_large` past 64 KiB of JSON, `409 too_many_preferences` past 100 keys in a slot. |
| Host `libraries[]` + library CRUD (`/hosts/{id}/libraries`) | **library-paths** | **frozen 2026-08-23.** `{ name, path, online, freeBytes, totalBytes, instanceCount, mount?, device? }`, replacing `installDirectory` (one root could not describe a host with two disks). **Three honesty rules:** `libraries:null` (engine cannot answer — the client hides placement) is NOT `[]` (registry read, nothing registered); `freeBytes`/`totalBytes` are `null` for an offline library, never `0` (a zero reads as a full disk); `online` is the engine's marker check while `mount`/`device` are the monitor's join, and neither is inferred from the other. Never cached — a root can be unplugged between two page loads. CRUD is viewer-read / admin-write through kgsm-lib's `ILibraryService`; **`DELETE` has no force** and serves the engine's refusal (which names the blocking instances) verbatim as a `409`; `?drain=<target>` moves every resident into the target first, which is the sanctioned way a disk is emptied, and blocks for the whole copy. `POST /servers` takes `library` (a NAME); an unknown or offline one is a synchronous `400`. `Server` gains `library`/`libraryPath`/`libraryState` (`online｜offline｜unregistered`, null only from an engine predating libraries — a SEPARATE axis from `status`, joined by the surface, never folded in; the engine measures it per invocation from the instance registry, which is the one read that works when the instance's own config cannot be opened). An instance on an OFFLINE library reports `status:"unknown"` and `runtime:null`, straight through: the engine reports `status:null` and omits `runtime`, and kgsm-lib carries both as nullables rather than as measured-looking defaults. `POST /servers/{id}/move { library }` is **admin** (placement shapes the host, so it takes the library-CRUD authority rather than operator) and returns `202` + a job whose verb is `move` — the engine starts the instance once on the new path to confirm it runs there, so run-state flickers `running` mid-move and the JOB's span is what a surface renders `moving` from. Four synchronous refusals (unknown server `404`; a library this host does not carry `400`; an offline target, the instance's own library, or a running instance `409`); free space is deliberately NOT among them, because the engine measures what the instance occupies before it copies and a second measurement here could disagree with the one that decides. Audit: `library.add`/`library.remove` and `server.move` (both libraries in `meta`) are engine echoes, `library.rename`/`library.failed` are direct writes because kgsm emits nothing for either, and a move the engine refused is a `command.failed` row carrying `fromLibrary`/`toLibrary`. |
...
kgsm-api/
  ...
  src/Api/
    ...
    Controllers/              # [M0] Health, Meta, Diagnostics · [M1·a] Hosts → [M1·b] Servers · [M2] Stream (SSE) → [M3] Commands → [M4·a] Auth → [M5] Audit → …
...
the canonical dev host (`/home/heisen/tks/kgsm`) with the API bound via `Api__Urls`
and a tunnel/allowlisted origin via `Api__CorsOrigins` — to confirm with the team.
...
(`Api__HostId`, default machine name) so it can't flap with the monitor; `status:"online"`
...
**Transport migrated WebSocket→SSE 2026-07-02** — this entry is the
2026-06-15 build log and describes the WebSocket transport as it was validated **at that time**; the
contract (topics, envelope, coalescing, capability model) carried over unchanged, only the wire
transport + auth (header bearer, not `?access_token=`) changed. See §6 for the current (SSE) contract.
...
**The stream (as built 2026-06-15; frozen contract now in §6).** Raw ASP.NET Core WebSockets at
`GET /api/v1/stream` (not SignalR — the hand-rolled `{topic,type,data}` envelope IS the contract).
Inbound `{type:subscribe|unsubscribe,topics[]}`; unknown command types and unknown/future topics
accepted silently (forward-compat for `jobs`/`audit`/…). Patch-only, no snapshot-on-subscribe (the
client hydrates via REST per §3·j). All topic/type strings are centralized in
`Realtime/StreamProtocol.cs` — none inline. Three gated pumps fan out through a per-host
...
byte-identical to the REST element it patches. **(2026-07-02: the inbound subscribe/unsubscribe channel
is gone — SSE has no client→server channel, so topics are chosen once via `?topics=` at connect,
immutable per connection; everything else in this paragraph carried over unchanged.)**
...
**Owed to the frontend at the gate:** connect a stream carrying the topics the active page needs
(close it on navigation away — post-2026-07-02, topics are chosen once via `?topics=` at connect, so
"unsubscribing" means closing that stream, not sending a command); treat `server.patch` data as the
full honest `Server` element (merge by id), `server.removed` as a drop; render a `capabilities.patch`
`down` (with `provisioned:true`) as "temporarily unavailable", not "gone", and snap back on the
`operational` flip; on stream drop, fall back to REST polling and re-hydrate on reconnect (§3·j). The
stream is **unauthenticated until M4** — same trusted-network window as M3.
...
explicit, loudly-logged `Api__AuthDisabled=true` escape hatch (synthetic admin — the pre-M4 open window).
...
checks run under `Api__AuthDisabled=true` (synthetic admin) so they exercise the domain contracts unchanged; a
...
**Honest boundary / op notes.** Dev ran with `Api__SigningKey` **blank → an ephemeral key** (logged
...
`Api__KgsmJournalDir` — no path to bind, nothing to reserve, and no risk of clobbering another consumer's
...
assistant** (`Api__FirewallSocketPath` blank ⇒ `absent`), and deliberately **NOT** in the 2s `LeafHealthMonitor`
...
with `Api__FirewallSocketPath` set, engine at the dev kgsm.
...
(//////) with an optional ops-role ping (`Settings["pingRoleId"]` + `allowed_mentions` scoped to that one
...
The instance file browser + editor behind kgsm-web's `FileBrowser` (`Services/Files` + `ServerFilesController`). **API-side
...
### M6·a increment 1 — 2026-06-30 · metrics-threshold alert source (host + per-server) — BUILT + unit-tested

**Status.** Per the approved design (2026-06-30): the deferred
`host-monitor`/`metrics` `AlertSource`s are now wired into the existing `AlertEngine`, so a sustained host or
per-server metric breach raises/resolves an alert through the **same** `/alerts` REST + `alerts` WS surface
M6·a already shipped for the crash source — no new endpoint, no protocol change. **BUILT + unit-tested
(build 0-warn; full suite 474/474, +10 new `TickMetrics` tests) + LIVE-VALIDATED 2026-07-01.** The live-fire ran
the dev build on `:8098` (auth-disabled, real `/run/kgsm-monitor/metrics.sock`) with low thresholds:
`metric:host-mem` raised **danger** (real 16% ≥ warn 10/danger 15; `host-monitor` source, `host` anchor, `serverId:null`,
`escalated:false` even in danger) and `metric:srv-pids:factorio-test` raised **warn** (30 pids ≥ 5; `metrics` source,
`server` anchor); stopping `factorio-test` via the deployed `:8097` dropped its monitor row → `metric:srv-pids` **resolved**
`by:system`, `actionId:null`, reason "no longer reporting metrics" (the vanished-row path) while `host-mem` kept firing
(independent conditions). factorio-test restored, dev process cleaned up. **Still UNCOMMITTED and not deployed to `:8097`;
`deploy.sh` + a frontend gate are owed.**

**What's being added.** A closed `ThresholdMetric` enum (host: `HostMemUsedPct` / `HostSwapUsedPct` /
`HostDiskUsedPct` (per-mount fan-out) / `HostLoadPerCore` / `HostTempC` (per-sensor fan-out); server:
`ServerMemBytes` / `ServerCpuPctCore` / `ServerPids`) + `ThresholdRule` (warn/danger/fireForSec/clearForSec/
clearMargin/enabled) + `MetricsThresholdPolicy`, in a new `Services/Alerts/ThresholdPolicy.cs` — the enum is
closed so a field that can't be honestly measured from the monitor `Snapshot` can never be thresholded.
`ApiOptions` gains `Policy` (bound from appsettings `MetricsThresholds:Rules`, env overrides, falls back to a
baked `MetricsThresholdPolicy.Default`) and `MetricsThresholdProvisioned`. `AlertDto` gains the
`AlertSurface.Host` constant (host-scope alerts deep-link to the host, not a server). `AlertEngine` is **not**
duplicated — it gains a `MonitorClient` dependency (already DI-registered for `MetricsPump`), a new
`internal TickMetrics(Snapshot? snap, DateTimeOffset now)` pass (the unit-test seam) run alongside the
unchanged `Tick` (crash) each poll, and both converge on one shared `RebuildSnapshot()` so the single-writer
invariant (`Services/Alerts/CLAUDE.md`) holds with crash and metric alerts living in the one `_firing`/`_resolved`.

**Locked decisions (full detail in `Services/Alerts/CLAUDE.md`):** policy = appsettings + env only at
increment 1 (DB-backed/panel-editable policy is increment 2, out of scope) — tuning a threshold means editing
config and restarting the API, the expected increment-1 UX, not a gap; host rules ship **ON** (universal `%`
thresholds), per-server rules ship **OFF** (absolute, game-specific — operator opt-in); alert id
`metric:<ruleKey>[:<ref-or-serverId>]` (a distinct namespace from `crash:<serverId>`; per-mount/per-sensor
rules fan out to one alert per target); anti-flap = an explicit fire-dwell (`_breachSince`, must breach
`FireForSec` before raising — crash didn't need this, its poll interval is its own debounce) plus the existing
clear-probation with a **hysteresis deadband** (`ClearMargin`) so a value hovering at the threshold can't
flap; honest-unknown = a down monitor (`snap==null`) skips the metric pass entirely (never resolves/retracts
on absence), a null field/null `Cpu.Info`/empty `Sensors` makes that rule not evaluable (hold), and a server
row that vanishes from a *live* (non-null) snapshot resolves after the normal clear-dwell; `resolution.actionId`
is **always `null`** for a metric alert (the actionId↔audit bridge is crash-specific — a metric clears because
the value receded, not an action), `resolution.by` stays `system`; a metric in the danger band still
auto-resolves, so metric alerts keep `Escalated:false` always — severity alone carries danger.

**Default policy (strawman, `appsettings.json` `MetricsThresholds`):** `host-disk` leads (90/95%, the single
most valuable alert — a full disk crashes servers), `host-mem` (90/97%), `host-swap` (50/90%), `host-load`
(1.5/4.0 per core), `host-temp` (85/95°C) all ship `enabled:true`; the three per-server rules (`srv-pids`,
`srv-mem`, `srv-cpu`) ship `enabled:false` (placeholder thresholds, inert until an operator opts in and tunes
them).

**Out of scope (increment 2+, unchanged from the plan):** a DB-backed/panel-editable policy store + admin
endpoint; new metrics beyond the closed enum (network-rate, io-saturation, players); any frontend work beyond
rendering the already-generic `Alert` record (a `host` anchor deep-link + two new source badges).

**Validation — done (unit + build), live-fire owed.** New `tests/Api.Tests/MetricsThresholdAlertTests.cs`
drives the `TickMetrics` seam — **all 10 green**: spike-under-dwell silent, sustained-breach fires after
`FireForSec`, danger re-push (raisedAt preserved), hysteresis no-flap, honest-null resolve (`by:system`,
`actionId:null`), null-field hold, monitor-down skip, vanished-row resolve-but-not-on-blackout,
per-mount fan-out, crash+metric coexistence (the crash pass leaves the metric alert untouched). Existing
`AlertEngineTests` stay green; **`dotnet build` 0-warn, `dotnet test` 474/474**. `scripts/smoke.sh` `/alerts`
contract unchanged (the 4 smoke failures on this host are pre-existing/environmental — a dirty audit DB, real
`factorio-test` console scrollback, and the M7 stub assistant not coming up — none touch alerts).

**Two fixes landed during the build (the tests caught them):**
1. **Config binding** — `MetricsThresholds:Rules` now binds through a mutable `ThresholdRuleBinding` DTO
   (settable properties → `ToRule()`), not the positional `ThresholdRule` record directly. The configuration
   binder cannot construct a record whose `double? Danger` ctor parameter has no value, and silently yields an
   empty list, so **any warn-only rule (`danger` absent/null) would drop the operator's whole custom policy back
   to `Default`** — and the default policy's own `srv-*` rules are warn-only, so the footgun was live. A rule
   missing its `Key` is skipped (malformed), never materialized with a null id.
2. **Shared-firing-set namespacing** — the crash `Tick` resolve/retract loop now guards `id.StartsWith("crash:")`.
   Crash and metric alerts share `_firing`; without the guard a watchdog poll sees a `metric:` alert's serverId
   absent from its `present`/`firingNow` sets and wrongly retracts/resolves a live metric condition.
