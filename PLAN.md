...
migrated WebSocket→SSE 2026-07-02, `sse-migration-plan.md`; frontend gate pending)
...
**M6·a — Alerts (condition-mirror).** · `partial` (crash source built & self-validated + **LIVE-VALIDATED** 2026-06-16; contract PROPOSED — frontend sign-off pending. **Metrics-threshold source (increment 1) BUILT + unit-tested + LIVE-VALIDATED 2026-07-01** per `metrics-threshold-alerts-plan.md` (474/474; dev instance vs real monitor: host-mem danger + srv-pids warn raise, vanished-row resolve — UNCOMMITTED, not yet deployed to :8097). See §6 + §8.)
...
- **Metric thresholds — increment 1, being wired (no longer deferred).** The `host-monitor`/`metrics`
  sources are being populated per `metrics-threshold-alerts-plan.md`: a sustained host/per-server
...
**⚠ Transport migrated WebSocket→SSE 2026-07-02** (`sse-migration-plan.md`) — this entry is the
...
**Status.** Per `tks/metrics-threshold-alerts-plan.md` (approved design, 2026-06-30): the deferred
