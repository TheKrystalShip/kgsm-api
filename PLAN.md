...
| Host `libraries[]` + library CRUD (`/hosts/{id}/libraries`) | **library-paths** | `../kgsm-library-paths-plan.md` §4 Phase 4 — **frozen 2026-08-23.** `{ name, path, online, freeBytes, totalBytes, instanceCount, mount?, device? }`, replacing `installDirectory` (one root could not describe a host with two disks). **Three honesty rules:** `libraries:null` (engine cannot answer — the client hides placement) is NOT `[]` (registry read, nothing registered); `freeBytes`/`totalBytes` are `null` for an offline library, never `0` (a zero reads as a full disk); `online` is the engine's marker check while `mount`/`device` are the monitor's join, and neither is inferred from the other. Never cached — a root can be unplugged between two page loads. CRUD is viewer-read / admin-write through kgsm-lib's `ILibraryService`; **`DELETE` has no force** and serves the engine's refusal (which names the blocking instances) verbatim as a `409`. `POST /servers` takes `library` (a NAME); an unknown or offline one is a synchronous `400`. `Server` gains `library`/`libraryPath`/`libraryState` (`online｜offline｜unregistered`, null when the registry is unreadable — a SEPARATE axis from `status`, joined by the surface, never folded in; read on the instance cache's own refresh so a server's record and its disk's state cannot disagree). ⚠ An instance on an OFFLINE library reports `status:"unknown"` and `runtime:null` — the engine reports `status:null` and omits `runtime`, and kgsm-lib's non-nullable `bool Status` / member-less `InstanceRuntime` turn both into measured-looking defaults (`stopped`/`native`), so the library check is what stands between a default and a fabrication. Audit: `library.add`/`library.remove` are engine echoes, `library.rename`/`library.failed` are direct writes because kgsm emits nothing for either. |
