...
### M7 — Assistant turn relay  ·  `partial` (backend built & self-validated 2026-06-19; the happy-path SSE stream + frontend gate pending)  ←  *resolves keystone O1*
...
- **Self-validated:** `scripts/smoke.sh` → **41/41** (+2 M7: assistant-absent → 404 capability gate,
  blank prompt → 400; the no-token sweep now covers `POST assistant/turn` → 401) + `tests/Api.Tests`
  → **109** (+4 `AssistantRelayTests`: no-token 401, none-tier 403, viewer+absent 404, blank-prompt 400);
  Release **0-warning**. The upstream §5·a Phase 1 is committed (kgsm-llm `bda373a`, suite 349 green).
- **Depends:** M4 (auth); the assistant service's **upstream §5·a pass** (`kgsm-llm/docs/m7-sse-5a-spec.md`,
  Phase 1) — **DONE** (committed `bda373a`).
- **Risk:** SSE relay/streaming correctness (the happy-path stream is the owed live-validate).
- **Coverage honesty (the live half, like M3's mutation / M5's socket):** the **happy-path SSE
  relay** — a real/stub assistant streaming §5·a frames back through the API verbatim — is a
  **trusted-host/stub live-validate** (it needs a running assistant + Ollama). The auth + capability
  gates that run *before* the upstream call are proven (smoke + tests); the byte-faithful relay +
  client-disconnect teardown are code-path-reviewed, owed an end-to-end run.
