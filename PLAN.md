...
- **Depends:** M4 (auth); the assistant service's **upstream §5·a pass** (`kgsm-llm/docs/m7-sse-5a-spec.md`,
  Phase 1) must land first — the readiness pass is now scoped, not open.
- **Risk:** SSE relay/streaming correctness; the upstream §5·a pass is the real prerequisite.
...
| Assistant SSE event vocabulary (proxy vs re-wrap) | M7 | `architecture.html §5·a`; keystone O1 — **RESOLVED 2026-06-19: near-verbatim relay, §5·a shaping pushed upstream into the assistant** (`kgsm-llm/docs/m7-sse-5a-spec.md`); **fork LOCKED (a)** — confirmed commands execute via M3, `command.verified` is NOT a turn event. Upstream Phase 1 is M7's prerequisite. |
