...
| 5 | Refresh + logout honor the session (revoke on logout; same-`sid` rotation on refresh) | ◪ **partial** (logout-revoke + refresh-session-check landed in 4·b) | logout server-side revoke + refresh reuse/validity check are DONE (see 4·b). Left: fold `RefreshResponse.expiresAt` (Inc 7) + a live OAuth soak. |
