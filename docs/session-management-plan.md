...
rotating `KGSM_API_AUTH_SIGNING_KEY`, which nukes **every** user (collateral blast). At 8h
...
| D8 | **Session TTL** | `Expires = Created + 30d` absolute. **No sliding** on refresh (the 30d cap stays absolute, per the original lock rationale). Refresh within the window re-mints access with the same `sid`; `LastSeen` updated. | **LOCKED:** preserves the absolute-cap intent |
| D9 | **Refresh rotation** | NOT in scope 1. The SPA keeps the same refresh token for 30d. Decision deferred to a later increment (would change `RefreshResponse` — a breaking wire change). | **LOCKED deferral:** scope 1 keeps `RefreshResponse` non-breaking (only adds `ExpiresAt`) |
...
|---|---|---|---|
| `KGSM_API_SESSIONS_DISABLED` | `false` | master switch; `true` → session registry inert, no per-request check, no revoke surface (escape hatch for debugging) | 1, 4, 6 |
| `KGSM_API_SESSIONS_CACHE_TTL_MS` | `5000` (floor `500`) | in-memory cache TTL (the revocation-lag bound) | 4 |
| `KGSM_API_SESSIONS_GC_MS` | `600000` (10 min) | how often the GC worker deletes expired rows | 8 |
| `KGSM_API_SESSIONS_REFRESH_ABSOLUTE_DAYS` | `30` | mirrors `SessionTokenService.RefreshTtl`; the `Expires` column is `Created + this`. (Lock doc note: stays in lockstep with the JWT refresh TTL — if you change one, change both.) | 1 |
...
- **Build:**
  ...
  - Master switch `KGSM_API_SESSIONS_DISABLED` plumbed into a small options class
...
- **Build:**
  - New `Services/Auth/ISessionValidator.cs` + `SessionValidator.cs`:
    ...
    - `IMemoryCache` (already registered by `AddMemoryCache()` if not present, add it in
      ...
      so a revoked session doesn't keep DB-hammering). TTL = `KGSM_API_SESSIONS_CACHE_TTL_MS`.
  ...
  - Extend `Startup.cs` `OnTokenValidated` (`:349-362`) — after the existing `tkn == access`
    ...
    if (KGSM_API_SESSIONS_DISABLED) return;               // escape hatch — honor it
...
- **Build:** new `Services/Auth/SessionCleanupWorker.cs` (`IHostedService` + `BackgroundService`,
  a `PeriodicTimer` with `KGSM_API_SESSIONS_GC_MS`):
  ...
  - Logged at debug; inert when `KGSM_API_SESSIONS_DISABLED`.
...
- **Stable signing key still required** — the 30d refresh still hinges on
  `KGSM_API_AUTH_SIGNING_KEY` being stable. Unchanged by this milestone.
